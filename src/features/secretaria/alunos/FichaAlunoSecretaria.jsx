...
// Ficha do Estudante — Módulo Secretaria (design premium independente)
...
export default function FichaAlunoSecretaria({ codigo }) {
  ...
  if (!aluno) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "3rem 1rem", gap: "0.75rem", color: "#94a3b8" }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid #e2e8f0", borderTop: "3px solid #3b82f6", animation: "fas-spin 0.8s linear infinite" }} />
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes fas-fadeUp { from { opacity:0; transform:translateY(12px); } to { opacity:1; transform:translateY(0); } }
        .fas-wrap { animation: fas-fadeUp 0.3s ease both; }
        .fas-ped:hover  { transform: translateY(-3px) !important; box-shadow: 0 14px 36px rgba(4,120,87,0.3) !important; }
        .fas-disc:hover { transform: translateY(-3px) !important; box-shadow: 0 14px 36px rgba(30,58,95,0.4) !important; }
      `}</style>

      <div className="fas-wrap" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
        {/* HEADER */}
        ...
        <div style={{ background:"#fff", borderRadius:"1.25rem 1.25rem 0 0", marginTop:"-1.5rem", padding:"1.25rem 1.5rem 1.5rem", position:"relative", zIndex:1, boxShadow:"0 -4px 20px rgba(0,0,0,0.07)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.875rem" }}>
            {/* Pedagógico */}
            <div className="fas-ped" onClick={() => setModalPedagogico(true)} style={{ background:"linear-gradient(135deg, #064e3b, #065f46)", border:"2px solid #059669", borderRadius:"1rem", padding:"1.1rem 1rem", cursor:"pointer", transition:"transform 0.2s, box-shadow 0.2s", boxShadow:"0 4px 16px rgba(4,120,87,0.2)" }}>
            ...
            {/* Disciplinar */}
            <div className="fas-disc" onClick={() => setModalDisciplinar(true)} style={{ background:"linear-gradient(135deg, #0f2044, #1e3a5f)", border:"2px solid #1d4ed8", borderRadius:"1rem", padding:"1.1rem 1rem", cursor:"pointer", transition:"transform 0.2s, box-shadow 0.2s", boxShadow:"0 4px 16px rgba(30,58,95,0.25)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"0.5rem", marginBottom:"0.45rem" }}>
                <span style={{ fontSize:"1.2rem" }}>📋</span>
                <span style={{ fontWeight:800, color:"#bfdbfe", fontSize:"0.9rem" }}>Relatório Disciplinar</span>
              </div>
              {ocorrenciasTotal === null
                ? <div style={{ display:"flex", alignItems:"center", gap:"0.4rem", color:"rgba(255,255,255,0.5)", fontSize:"0.75rem" }}><div style={{ width:12, height:12, borderRadius:"50%", border:"2px solid rgba(255,255,255,0.2)", borderTop:"2px solid #93c5fd", animation:"fas-spin 0.8s linear infinite" }} /><span>Carregando...</span></div>
