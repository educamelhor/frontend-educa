  ...
  return (
    <aside className={`sidebar-aside ${isOpen ? 'sidebar-open' : ''}`}>
      <nav className="p-4">
        ...
        {/* ─── GRUPO: Secretaria (Professor e Militar NÃO têm acesso; exclusivo do escopo Escola) ─── */}
        {isScopeEscola && !isDisciplinar && !isProfessor && hasModulo('secretaria') && (
          <>
            <button
              className="flex items-center w-full py-2 px-3 rounded hover:bg-blue-700 mt-6 transition"
        ...
        {/* ────────────────────────────────────────────────────
            GRUPO: COMUNICAÇÃO
            Acesso: TODOS os usuários da escola, EXCETO militares
            Guard: isScopeEscola + !isDisciplinar + hasModulo('comunicacao')
        ──────────────────────────────────────────────────── */}
        {isScopeEscola && !isDisciplinar && hasModulo('comunicacao') && (
          <>
            <button
              className="flex items-center w-full py-2 px-3 rounded hover:bg-blue-700 mt-2 transition"
              onClick={() => setOpenGroup(openGroup === 'comunicacao' ? null : 'comunicacao')}
              type="button"
              style={{
                background: openGroup === 'comunicacao'
                  ? 'linear-gradient(90deg, rgba(99,102,241,0.18), transparent)'
                  : undefined,
              }}
            >
              ...
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none"
                ...
                style={{ color: openGroup === 'comunicacao' ? '#818cf8' : undefined }}
              >
                <path strokeLinecap="round" strokeLinejoin="round"
                  d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
              </svg>
              <span className="flex-1 text-left" style={{ fontWeight: 700 }}>Comunicação</span>
              <span style={{
                fontSize: '0.55rem',
                fontWeight: 800,
                background: 'linear-gradient(135deg, #6366f1, #8b5cf6)',
            ...
            {openGroup === 'comunicacao' && (
              <ul className="ml-4 mb-2">
                {hasModulo('comunicacao.avisos') && (
                <li>
                  <Link
                    to="/comunicacao/avisos"
                    className={getSubmenuLinkClasses('/comunicacao/avisos')}
                    style={{
                      background: isActive('/comunicacao/avisos')
                        ? 'linear-gradient(90deg, rgba(99,102,241,0.18), transparent)'
                        : undefined,
                    }}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none"
                      viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
                      style={{ color: isActive('/comunicacao/avisos') ? '#818cf8' : undefined }}
                    >
                      <path strokeLinecap="round" strokeLinejoin="round"
                ...
                {hasModulo('comunicacao.comunicados') && (
                <li>
                  <Link
                    to="/comunicacao/comunicados"
                    className={getSubmenuLinkClasses('/comunicacao/comunicados')}
                    style={{
                      background: isActive('/comunicacao/comunicados')
                        ? 'linear-gradient(90deg, rgba(14,165,233,0.15), transparent)'
                        : undefined,
                    }}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none"
                      viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
                      style={{ color: isActive('/comunicacao/comunicados') ? '#38bdf8' : undefined }}
                    >
                      <path strokeLinecap="round" strokeLinejoin="round"
                ...
                {hasModulo('comunicacao.mural') && (
                <li>
                  <Link
                    to="/comunicacao/mural"
                    className={getSubmenuLinkClasses('/comunicacao/mural')}
                    style={{
                      background: isActive('/comunicacao/mural')
                        ? 'linear-gradient(90deg, rgba(16,185,129,0.15), transparent)'
                        : undefined,
                    }}
                  >
