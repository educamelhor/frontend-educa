...
export default function FichaAlunoSecretaria({ codigo: codigoProp }) {
  ...
  const processAndUpload = async (arquivoFonte) => {
    ...
    try {
      ...
      const resp = await api.post(`/api/alunos/${codigo}/foto`, formData, { headers: { "Content-Type": "multipart/form-data" } });
    ...
    return (
      <div style={{ padding: 40, display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: isModal ? 200 : '100vh' }}>
        <div style={{ textAlign: 'center', color: '#6b7280' }}>
          <div style={{ width: 36, height: 36, border: '3px solid #e5e7eb', borderTopColor: '#b45309', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
  ...
  return (
    <div style={{ fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      <div style={{ background: 'linear-gradient(135deg, #78350f 0%, #92400e 60%, #b45309 100%)', borderRadius: '16px 16px 0 0', padding: '24px 24px 20px', color: '#fff' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ flexShrink: 0 }}>
            {fotoURL ? (
              ...
              <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg, #f59e0b, #78350f)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 700, color: '#fff', border: '3px solid rgba(255,255,255,0.3)', letterSpacing: '-1px' }}>{iniciais}</div>
            )}
          </div>
          <div style={{ flex: 1 }}>
      ...
      <div style={{ background: '#fff', borderRadius: '0 0 16px 16px', padding: '20px 24px 24px' }}>
        ...
        <div style={{ marginBottom: 20, padding: '12px 14px', background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 10 }}>
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, cursor: uploading ? 'not-allowed' : 'pointer' }}>
            <span style={{ padding: '7px 14px', background: uploading ? '#e5e7eb' : '#78350f', color: '#fff', borderRadius: 8, fontSize: 12, fontWeight: 600, transition: 'background 0.2s' }}>{uploading ? "Enviando…" : "Escolher pasta"}</span>
