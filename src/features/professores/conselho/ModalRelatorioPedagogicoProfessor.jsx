...
// ============================================================================
// Relatório Pedagógico — perfil PROFESSOR (isolado)
//
// Governança:
//  ✅ Professor PODE visualizar registros pedagógicos
//  ✅ Professor PODE adicionar novas ocorrências
//  ❌ Campo Descrição         — OCULTO
//  ❌ Campo Registro Interno  — OCULTO
//  ❌ Editar/Excluir/Finalizar registros de outros — OCULTO
//
...
export default function ModalRelatorioPedagogicoProfessor({ open, onClose, aluno }) {
  ...
  useEffect(() => {
    if (open && aluno?.id) fetchOcorrencias();
  ...
  const fetchOcorrencias = async () => {
    ...
    try {
      ...
      console.error("[RelPedProf] Erro ao carregar registros:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <div className="mb-4 px-4 py-2 bg-blue-50 border border-blue-200 rounded text-sm text-blue-700">
            ...
            Os campos <strong>Descrição</strong> e <strong>Registro Interno</strong> são exclusivos da coordenação e direção.
          </div>
          ...
          <div className="flex items-center gap-6 mb-6 p-4 bg-emerald-50/50 border border-emerald-100 rounded-lg">
            ...
            <div className="flex flex-col gap-0.5 flex-1 min-w-0">
              <h3 className="text-xl font-bold text-gray-800 uppercase truncate">{aluno.estudante ?? "NOME NÃO INFORMADO"}</h3>
              <p className="text-sm text-gray-600">
                <span className="font-semibold">Turma:</span> {aluno.turma ?? "-"} {aluno.turno ? `(${aluno.turno})` : ""}
              </p>
              <p className="text-xs text-gray-500">
                <span className="font-semibold">Código:</span> {aluno.codigo}
              </p>
            </div>
            ...
            <div className="flex-shrink-0">
              <button
                onClick={() => { setOcorrenciaSelecionada(null); setModoVisualizacao(false); setNovaOcorrenciaOpen(true); }}
          ...
          <div className="border rounded-lg overflow-hidden shadow-sm">
            <table className="min-w-full text-left bg-white text-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {loading ? (
                  <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500 italic">Carregando...</td></tr>
                ) : ocorrencias.length === 0 ? (
                  <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500 italic">Nenhum registro pedagógico encontrado.</td></tr>
                ) : (
                  ocorrencias.map((oc) => (
                    <tr key={oc.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() => { setOcorrenciaSelecionada(oc); setModoVisualizacao(true); setNovaOcorrenciaOpen(true); }}
      ...
      <ModalNovaOcorrenciaPedagogicaProfessor
        ...
        onClose={() => { setNovaOcorrenciaOpen(false); setOcorrenciaSelecionada(null); setModoVisualizacao(false); }}
        aluno={aluno}
