...
export default function Professores() {
  ...
  useEffect(() => {
    api
      .get("/api/professores") // ← corrigido: agora com /api
      .then((resp) => {
  ...
  function handleDelete(prof) {
    ...
    api
      .delete(`/api/professores/${prof.cpf}`) // ← corrigido: agora com /api
      .then(() => {
  ...
  function handleEdit(prof) {
    ...
    api
      .put(`/api/professores/${prof.cpf}`, { // ← corrigido: agora com /api
        ...prof,
        nome: novoNome,
  ...
  // 4) filtro de busca (inclui Turno)
  const termo = (filtro || "").toLowerCase();
  const listaFiltrada = (professores || []).filter((p) =>
    (p.nome || "").toLowerCase().includes(termo) ||
    (p.cpf || "").includes(termo) ||
    (p.turno || "").toLowerCase().includes(termo) ||
  ...
  return (
    <div className="p-6 bg-blue-50 min-h-screen space-y-6">
      ...
      <div className="flex justify-between items-center">
        <input
          type="text"
          placeholder="🔍 Buscar professor (CPF, Nome, Turno, Disciplina)"
            ...
            {listaFiltrada.length > 0 ? (
              listaFiltrada.map((prof) => (
                <tr key={prof.cpf} className="hover:bg-blue-50">
                  ...
                  <td className="p-3 border">{prof.turno || "—"}</td>
