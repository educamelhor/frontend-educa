...
export default function DisciplinaForm({ open, onClose, onSubmit, disciplina }) {
  const [form, setForm] = useState({ nome: '', carga: '', etapa: 'FUNDAMENTAL' });
  ...
  useEffect(() => {
    ...
    if (disciplina) {
      ...
      setForm({ id: null, nome: '', carga: '', etapa: 'FUNDAMENTAL' });
      setErrors({});
    }
  }, [open, disciplina]);
        ...
        >
          ...
          <div style={{ padding: '24px 28px' }}>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <input
                ...
                style={{
                  width: '100%',
                  padding: '11px 14px',
                  border: errors.nome ? '2px solid #ef4444' : '1.5px solid #d1d5db',
                  borderRadius: 10,
                  fontSize: 15,
                  color: '#111827',
                  background: '#fff',
                ...
                onBlur={e => { e.target.style.borderColor = errors.nome ? '#ef4444' : '#d1d5db'; e.target.style.boxShadow = 'none'; }}
              />
              {errors.nome && (
                <p style={{ margin: '4px 0 0', fontSize: 12, color: '#ef4444' }}>⚠ {errors.nome}</p>
              )}
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ position: 'relative' }}>
                <select
                  ...
                  style={{
                    ...
                    border: errors.etapa ? '2px solid #ef4444' : '1.5px solid #d1d5db',
                    borderRadius: 10,
                    fontSize: 15,
                    color: '#111827',
                    background: '#fff',
                  ...
                  onFocus={e => { e.target.style.borderColor = '#3b82f6'; e.target.style.boxShadow = '0 0 0 3px rgba(59,130,246,0.12)'; }}
                  onBlur={e => { e.target.style.borderColor = errors.etapa ? '#ef4444' : '#d1d5db'; e.target.style.boxShadow = 'none'; }}
                >
                  {ETAPAS.map(op => (
              ...
              {errors.etapa && (
                <p style={{ margin: '4px 0 0', fontSize: 12, color: '#ef4444' }}>⚠ {errors.etapa}</p>
              )}

              {/* Preview badge da etapa selecionada */}
              {form.etapa && (
                <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{
                    ...
                    fontSize: 12,
                    fontWeight: 600,
                    background: etapaCor.bg,
                    color: etapaCor.color,
                    border: `1px solid ${etapaCor.border}`,
                  }}>
                    {ETAPAS.find(e => e.value === form.etapa)?.label}
                  </span>
                  {form.nome && (
                    <span style={{ fontSize: 12, color: '#6b7280' }}>
                      → será exibido como <strong>"{form.nome.trim()}"</strong> no boletim
                    </span>
                  )}
                </div>
              )}
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{
                    ...
                    border: errors.carga ? '2px solid #ef4444' : '1.5px solid #d1d5db',
                    borderRadius: 10,
                    fontSize: 15,
                    color: '#111827',
                    background: '#fff',
                  ...
                  onFocus={e => { e.target.style.borderColor = '#3b82f6'; e.target.style.boxShadow = '0 0 0 3px rgba(59,130,246,0.12)'; }}
                  onBlur={e => { e.target.style.borderColor = errors.carga ? '#ef4444' : '#d1d5db'; e.target.style.boxShadow = 'none'; }}
                />
                <span style={{
              ...
              {errors.carga && (
                <p style={{ margin: '4px 0 0', fontSize: 12, color: '#ef4444' }}>⚠ {errors.carga}</p>
              )}
            </div>
          </div>
