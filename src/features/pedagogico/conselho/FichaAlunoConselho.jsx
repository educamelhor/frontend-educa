// src/features/pedagogico/conselho/FichaAlunoConselho.jsx
// ============================================================================
// Ficha do Estudante — Visão Direção / Coordenação (arquivo INDEPENDENTE)
// - Layout premium (mesmo padrão de FichaAlunoProfessor)
// - Sem botão "Escolher Pasta" / sem label "Selecionar Pasta e Inserir Foto"
// - Card Relatório Pedagógico: clicável (abre ModalRelatorioPedagogico)
// - Card Relatório Disciplinar: clicável (acesso total — direção/coordenação)
// ============================================================================
...
function disciplinarBadge(total) {
  if (total === 0) return { label: "Sem ocorrências", bg: "rgba(255,255,255,0.12)", color: "#d1fae5", icon: "✅" };
  if (total <= 2)  return { label: `${total} ocorrência${total > 1 ? "s" : ""}`, bg: "rgba(255,255,255,0.12)", color: "#fde68a", icon: "⚠️" };
  if (total <= 5)  return { label: `${total} ocorrências`, bg: "rgba(255,255,255,0.12)", color: "#fed7aa", icon: "🔶" };
  return             { label: `${total} ocorrências`, bg: "rgba(255,255,255,0.12)", color: "#fecaca", icon: "🔴" };
}
...
export default function FichaAlunoConselho({ codigo }) {
  ...
  if (erro) return (
    <div style={{ padding: "2rem", color: "#b91c1c", textAlign: "center" }}>{erro}</div>
  );

  if (!aluno) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "3rem 1rem", gap: "0.75rem", color: "#94a3b8" }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid #e2e8f0", borderTop: "3px solid #3b82f6", animation: "fac-spin 0.8s linear infinite" }} />
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes fac-fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
        .fac-wrap { animation: fac-fadeUp 0.3s ease both; }
        .fac-ped:hover  { transform: translateY(-3px) !important; box-shadow: 0 14px 36px rgba(4,120,87,0.3) !important; }
        .fac-disc:hover { transform: translateY(-3px) !important; box-shadow: 0 14px 36px rgba(30,58,95,0.4) !important; }
      `}</style>

      <div className="fac-wrap" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.875rem" }}>
            ...
            >
              ...
              <p style={{ color: "#6ee7b7", fontSize: "0.75rem", margin: "0 0 0.5rem", lineHeight: 1.4 }}>
                Histórico completo de registros pedagógicos do estudante.
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", color: "#34d399", fontSize: "0.73rem", fontWeight: 700 }}>
                <span>Ver histórico</span><span>→</span>
              </div>
            </div>
            ...
            <div
              className="fac-disc"
              onClick={() => setModalDisciplinar(true)}
              style={{
            ...
            >
              ...
              {ocorrenciasTotal === null ? (
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", color: "rgba(255,255,255,0.5)", fontSize: "0.75rem" }}>
                  <div style={{ width: 12, height: 12, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.2)", borderTop: "2px solid #93c5fd", animation: "fac-spin 0.8s linear infinite" }} />
                  <span>Carregando...</span>
                </div>
              ) : (
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", marginBottom: "0.5rem" }}>
                  <span style={{ fontSize: "0.9rem" }}>{badge.icon}</span>
                  <span style={{ fontWeight: 700, color: badge.color, fontSize: "0.82rem" }}>{badge.label}</span>
                </div>
              )}

              <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", color: "#93c5fd", fontSize: "0.73rem", fontWeight: 700 }}>
                <span>Abrir relatório</span><span>→</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {modalDisciplinar && (
        <ModalRelatorioDisciplinar
          aluno={aluno}
          onClose={() => setModalDisciplinar(false)}
        />
      )}
    </>
  );
