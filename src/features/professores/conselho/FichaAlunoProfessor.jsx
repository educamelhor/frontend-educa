...
// Ficha do Estudante — Professor (independente)
// Layout idêntico ao FichaAlunoConselho, mas Disciplinar NÃO é clicável
  ...
  return (
    <>
      <style>{`
        ...
        .fap-ped:hover { transform: translateY(-3px) !important; box-shadow: 0 14px 36px rgba(4,120,87,0.3) !important; }
      `}</style>
      ...
      <div className="fap-wrap" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
        ...
        <div style={{ background:"#fff", borderRadius:"1.25rem 1.25rem 0 0", marginTop:"-1.5rem", padding:"1.25rem 1.5rem 1.5rem", position:"relative", zIndex:1, boxShadow:"0 -4px 20px rgba(0,0,0,0.07)" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.875rem" }}>
            {/* Pedagógico — clicável */}
            <div className="fap-ped" onClick={() => setModalPedagogico(true)} style={{ background:"linear-gradient(135deg, #064e3b, #065f46)", border:"2px solid #059669", borderRadius:"1rem", padding:"1.1rem 1rem", cursor:"pointer", transition:"transform 0.2s, box-shadow 0.2s", boxShadow:"0 4px 16px rgba(4,120,87,0.2)" }}>
            ...
            {/* Disciplinar — visual idêntico mas NÃO clicável (acesso restrito) */}
            <div style={{ background:"linear-gradient(135deg, #0f2044, #1e3a5f)", border:"2px solid #1d4ed8", borderRadius:"1rem", padding:"1.1rem 1rem", cursor:"not-allowed", boxShadow:"0 4px 16px rgba(30,58,95,0.25)", opacity:0.85 }}>
              <div style={{ display:"flex", alignItems:"center", gap:"0.5rem", marginBottom:"0.45rem" }}>
                <span style={{ fontSize:"1.2rem" }}>📋</span>
                <span style={{ fontWeight:800, color:"#bfdbfe", fontSize:"0.9rem" }}>Relatório Disciplinar</span>
              </div>
              {ocorrenciasTotal === null
                ? <div style={{ display:"flex", alignItems:"center", gap:"0.4rem", color:"rgba(255,255,255,0.5)", fontSize:"0.75rem" }}><div style={{ width:12, height:12, borderRadius:"50%", border:"2px solid rgba(255,255,255,0.2)", borderTop:"2px solid #93c5fd", animation:"fap-spin 0.8s linear infinite" }} /><span>Carregando...</span></div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"0.35rem", color:"rgba(147,197,253,0.5)", fontSize:"0.73rem", fontWeight:600 }}><span>🔒</span><span>Acesso restrito à direção</span></div>
            </div>
          </div>
        </div>
      </div>
